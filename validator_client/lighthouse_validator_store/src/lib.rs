...
use types::{
    ...
    graffiti::GraffitiString, PublicKey, KzgProofs, BlobsList
};
use validator_store::{
    ...
    async fn sign_abstract_block<Payload: AbstractExecPayload<E>>(
        ...
        if signing_method.responsible(signing_epoch).await {
            ...
            match slashing_status {
                ...
                Ok(Safe::Valid) => {
                    signing_method
                        .distributed_propose_block(domain_hash, &block)
                        .await;
...
impl<T: SlotClock + 'static, E: EthSpec> LighthouseValidatorStore<T, E> {
    ...
    pub fn set_validator_fee_recipient(
            ...
            .set_validator_fee_recipient(validator_public_key, fee_recipient)
        {
            Ok(_) => {}
            Err(e) => {
                error!(error=?e, "failed to set validator fee recipient");
            }
        }
    }
    ...
    pub async fn enable_keystore(&self, validator_public_key: &PublicKey) {
            ...
            .await
        {
            Ok(()) => {}
            Err(e) => {
                error!(error=?e, "start validator keystore failed");
            }
        }
    }
    ...
    pub async fn disable_keystore(&self, validator_public_key: &PublicKey) {
            ...
            .await
        {
            Ok(()) => {}
            Err(e) => {
                error!(error=?e, "stop validator keystore failed");
            }
        }
    }
}
    ...
    async fn sign_block(
        ...
        match block {
            UnsignedBlock::Full(block) => {
                ...
                let maybe_blobs = blobs.clone().map(|(proofs, list)| (proofs, list));
                self.sign_abstract_block(validator_pubkey, block, current_slot, maybe_blobs)
                    .await
                    .map(|block| {
            ...
            UnsignedBlock::Blinded(block) => self
                .sign_abstract_block(validator_pubkey, block, current_slot, None)
                .await
        ...
        if signing_method.responsible(signing_epoch).await {
            ...
            match slashing_status {
                ...
                Ok(Safe::Valid) => {
                    ...
                    let validator_index = self.validators.read().get_index(&validator_pubkey)
                        .ok_or(Error::UnknownPubkey(validator_pubkey))?;
