        ...
        match self {
            SigningMethod::DistributedKeystore {
                ...
                match block_type {
                    BlockType::Blinded => {
                        operator_committee
                            .propose_blinded_block(&data, domain_hash)
                            .await;
                    }
                    BlockType::Full => {
                        operator_committee
                            .propose_full_block(&data, domain_hash)
                            .await;
                    }
                };
            }
    ...
    pub async fn get_signature_from_root<E: EthSpec, Payload: AbstractExecPayload<E>>(
        ...
        match self {
            ...
            SigningMethod::DistributedKeystore {
                ...
                let _timer = validator_metrics::start_timer_vec(
                    &validator_metrics::SIGNING_TIMES,
                    &[validator_metrics::DISTRIBUTED_KEYSTORE],
                );
                let (epoch, slot, duty, only_aggregator) = match signable_message {
                    ...
                    SignableMessage::AttestationData(a) => {
                        (a.slot.epoch(E::slots_per_epoch()), a.slot, "ATTESTER", true)
                    }
                    SignableMessage::BeaconBlock(b) => (
                        b.slot().epoch(E::slots_per_epoch()),
                        b.slot(),
                        "PROPOSER",
                        true,
                    ),
                    SignableMessage::SignedAggregateAndProof(x) => (
                        x.aggregate().data().slot.epoch(E::slots_per_epoch()),
                        x.aggregate().data().slot,
                        "AGGREGATE",
                        true,
                    ),
                    ...
                    SignableMessage::SyncSelectionProof(s) => (
                        s.slot.epoch(E::slots_per_epoch()),
                        s.slot,
                        "SYNC_SELECT",
                        false,
                    ),
                    ...
                    SignableMessage::SignedContributionAndProof(c) => (
                        c.contribution.slot.epoch(E::slots_per_epoch()),
                        c.contribution.slot,
                        "CONTRIB",
                        true,
                    ),
                    SignableMessage::ValidatorRegistration(_) => {
                        (Epoch::new(0), Slot::new(0), "VA_REG", false)
                    }
                    SignableMessage::VoluntaryExit(e) => (
                        e.epoch,
                        e.epoch.start_slot(E::slots_per_epoch()),
                        "VA_EXIT",
                        true,
                    ),
                };

                let is_aggregator = operator_committee.is_leader(epoch.as_u64())
                ...
                store_sender
                    .send((
                        ...
                        operator_committee.validator_public_key.clone(),
                    ))
                    .await
                    .unwrap();
