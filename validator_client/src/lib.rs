...
use safestake_service::operator_service::{SafestakeService, get_validator_keys, get_channels};
    ...
    pub async fn new(context: RuntimeContext<E>, config: Config) -> Result<Self, String> {
        ...
        if !config.disable_auto_discover {
            let new_validators = validator_defs
                .discover_distributed_keystores(&config.validator_dir, &config.secrets_dir)
        ...
        let operator_channels = Arc::new(RwLock::new(get_channels(&validator_defs, &config.safestake_config)?));
