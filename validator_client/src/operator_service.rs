use crate::operator::proto::safestake_server::Safestake;
use crate::operator::proto::{CheckLivenessRequest, CheckLivenessResponse, GetSignatureRequest, GetSignatureResponse, AttestRequest, AttestResponse, ProposeFullBlockRequest, ProposeFullBlockResponse, ProposeBlindedBlockRequest, ProposeBlindedBlockResponse};
use std::collections::HashMap;
use tonic::{Request, Response, Status};
use bls::{PublicKey, Keypair, Signature as BlsSignature};
use safestake_crypto::secret::Secret;
use safestake_crypto::secp::{Digest, Signature};
use store::{LevelDB, KeyValueStore};
use std::sync::Arc;
use tokio::sync::mpsc::Receiver;
use parking_lot::RwLock;
use dvf_utils::VERSION;
use types::{Hash256, AttestationData, BlindedPayload, EthSpec, BeaconBlock, AbstractExecPayload, FullPayload, ExecPayload};
use slashing_protection::{SlashingDatabase, Safe, NotSafe};
...
impl<E: EthSpec> SafestakeService<E> {
    ...
    async fn check_version_and_validator_public_key(
        ...
        if version != VERSION {
            return Err(Status::internal(format!("version mismatch, expected {}, got {}", VERSION, version)));
        }
        let validator_public_key = PublicKey::deserialize(validator_public_key).map_err(|e| {
            Status::internal(format!("failed to deserialize validator public key {:?}", e))
        })?;
        if !self.keypairs.read().contains_key(&validator_public_key) {
            return Err(Status::internal(format!("unkown validators {}", validator_public_key)));
        }
    ...
    fn check_operator_domain_hash_signature(
        ...
        let signature = Signature::from_bytes(signature).map_err(|e| {
            Status::internal(format!("failed to deserialize signature {:?}", e))
        })?;
        let operator_public_key = self.safestake_database.with_transaction(|tx| {
            self.safestake_database.query_operator_public_key(tx, operator_id)
        }).map_err(|e| {
            Status::internal(format!("failed to find operator's public key {:?}", e))
        })?.ok_or(Status::internal(format!("failed to find operator's public key")))?;

        signature.verify(&Digest(domain_hash.0), &operator_public_key).map_err(|_| {
            Status::internal(format!("failed to verify operator's signature"))
        })?;
    ...
    async fn sign_block<Payload: AbstractExecPayload<E>>(
        ...
        Ok(match self.slashing_database.check_and_insert_block_proposal(
            ...
            Ok(Safe::Valid) => {
                ...
                let keypair = keypairs.get(validator_public_key).ok_or(Status::internal(format!("unkown validator public key {}", validator_public_key)))?;

                let sig = keypair.sk.sign(signing_root);
                let serialized_signature = bincode::serialize(&sig).unwrap();

                self.store.put_bytes(&validator_public_key.as_hex_string(), signing_root.as_bytes(), &serialized_signature).map_err(|e| {
            ...
            Err(e) => {
                format!("do not sign slashable proposal {}: {:?}", validator_public_key, e)
            }
        })
    }
}
...
impl<E: EthSpec> Safestake for SafestakeService<E> {
    async fn check_liveness(
        ...
        let _ = self.check_version_and_validator_public_key(req.version, &req.validator_public_key).await?;
        if req.msg.len() != 32 {
            return Err(Status::internal(format!("invalid message length")));
        }
        let msg: [u8; 32] = req.msg.try_into().unwrap();
        let sig = Signature::new(&Digest::from(&msg), &self.secret.secret).map_err(|e| {
    ...
    async fn get_signature(
        ...
        let req = request.into_inner();
        let _ = self.check_version_and_validator_public_key(req.version, &req.validator_public_key).await?;
        let signature = self.store.get_bytes(&hex_encode(&req.validator_public_key), &req.msg).map_err(|e| {
        ...
        if let Some(signature) = signature {
            ...
            Err(Status::internal(format!("failed to find message's signature")))
        }
    }
    ...
    async fn attest_data(
        ...
        let req = request.into_inner();
        let validator_public_key = self.check_version_and_validator_public_key(req.version, &req.validator_public_key).await?;

        let domain_hash = Hash256::from(&req.domain_hash.try_into().unwrap());
        self.check_operator_domain_hash_signature(&domain_hash, &req.domian_hash_signature, req.operator_id)?;

        let attestation_data: AttestationData =
        match serde_json::from_slice(&req.attestation_data) {
            Ok(a) => a,
            Err(e) => {
                error!(self.logger, "deserialize attestation data"; "error" => %e);
                return Err(Status::internal(format!("failed to deserialize attestation data")));
            }
        };
        ...
        let output = match self.slashing_database.check_and_insert_attestation(
            ...
            Ok(Safe::Valid) => {
                ...
                let sig = keypair.sk.sign(signing_root);
                let serialized_signature = bincode::serialize(&sig).unwrap();
                self.store.put_bytes(&hex_encode(&req.validator_public_key), signing_root.as_bytes(), &serialized_signature).map_err(|e| {
                    Status::internal(format!("failed to save signature {:?}", e))
                })?;
            ...
            Err(NotSafe::UnregisteredValidator(validator_public_key)) => {
                format!("do not signing attestation for unregistered validator public_key {}", validator_public_key)
            }
            Err(e) => {
                format!("do not sign slashable attestation {}: {:?}", validator_public_key, e)
            }
        };
    ...
    async fn propose_full_block(
        ...
        let block: BeaconBlock<E, FullPayload<E>> = match serde_json::from_slice(&req.full_block_data) {
            Ok(b) => b,
            Err(e) => {
                error!(self.logger, "deserialize full block"; "error" => %e);
                return Err(Status::internal(format!("failed to deserialize propose full block data")));
            }
        };

        let fee_recipient = self.safestake_database.with_transaction(|tx| {
            self.safestake_database.query_validator_fee_recipient(tx, &validator_public_key)
        }).map_err(|e| {
        ...
        if fee_recipient.as_slice() != &block_fee_recipient.0 {
            return Err(Status::internal(format!("fee recipient mismatch, local fee recipient {:?}, block fee recipient {:?}", fee_recipient, block_fee_recipient)));
        }
        ...
        let output = self.sign_block(block, domain_hash, &validator_public_key).await?;

        Ok(Response::new(ProposeFullBlockResponse {
            msg: output,
        }))
    }
    ...
    async fn propose_blinded_block(
        ...
        let block: BeaconBlock<E, BlindedPayload<E>> = match serde_json::from_slice(&req.blinded_block_data) {
            Ok(b) => b,
            Err(e) => {
                error!(self.logger, "deserialize blinded block"; "error" => %e);
                return Err(Status::internal(format!("failed to deserialize propose blinded block data")));
            }
        };
        ...
        Ok(Response::new(ProposeBlindedBlockResponse {
            msg: output,
        }))
    }
}